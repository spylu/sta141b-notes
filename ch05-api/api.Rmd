...
<<<<<<< HEAD
# you can change timezone using `with_tz`
as_datetime(citibike$last_updated)
with_tz(as_datetime(citibike$last_updated))
```
```{r}
# gives us snapshot of data
=======
...
```{r}
>>>>>>> 90613f353e14c98435d14e3f0ed50c35962b1622
citibike %>% glimpse()
```
...
```{r, cache = TRUE}
# this api tells you whether there is water at some given coordinates
# davis
<<<<<<< HEAD
# same as manually inserting lat and long by hand
...
```{r}
facebook <- fromJSON("http://api.domainsdb.info/v1/domains/search?domain=facebook")
# you don't want to directly use `GET` on this object because it's messy 
```

```{r}
# setting `GET` parameter
# stick with this approach for rest of APIs
r <- GET(
"http://api.domainsdb.info/v1/domains/search",
query = list(
domain = "facebook"
))
```
...
```{r}
# do this to check whether request was successful
r
# converts results from JSON to R list, doesn't use JSONlite package so no simplification into nicer objects
content(r)
```
...
```{r}
username <- "randy3k"
# need this `GET` parameter to go to next page
r <- GET(
...
<<<<<<< HEAD
Different APIs use different strategies so we need to know how to access different kinds.
...
```{r}
# in this `GET` command you can search based on other things, doesn't have to be `location`
r <- GET(
...
stop_for_status(r)
json <- content(r, as = "text")
# without `select`, will see all the data for each business in Davis in JSON file
# this tells you just the names of all the businesses in Davis
...
endpoint <- str_glue(
  ...
  term = "statistics")
# since Noun Project uses one-legged OAuth protocol, need endpoint and append signature
...
# this gets you cute icons in black-ish color
# if you want second page will need to specify `GET` parameter
...
# with access token, you can access twitter's API
# new access token generated when you run this `twitter_token` thing
twitter_token <- oauth2.0_token(
  oauth_endpoint(
    authorize = NULL,
# will need to copy some URLs to get token
...
```{r}
# this lets you get the access token
twitter_token$credentials$access_token
```

This document teaches you to get API stuff manually, and though you can use packages for `get_woeid`, etc. in your project, it's not too difficult to write your own and you'll learn more.
